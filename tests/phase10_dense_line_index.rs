...
fn dense_line_index_reads_final_line_without_newline() {
    let input = b"alpha\nbeta\ngamma";
    let container = pack_bytes_with_dense_line_index(input, [0xa0; 16], options(8, 8))
...
fn dense_line_index_count_mismatch_is_rejected() {
    let input = b"alpha\nbeta\n";
    let mut container = pack_bytes_with_dense_line_index(input, [0xa1; 16], options(32, 32))
        .expect("dense container should pack");
...
fn deep_verify_detects_dense_line_index_disagreement() {
    ...
    let container =
        pack_bytes_with_dense_line_index_override(input, [0xa2; 16], options(64, 64), dense)
...
fn sparse_vs_dense_line_lookup_benchmark_records_threshold_evidence() {
    ...
    let sparse_container =
        qzt::writer::pack_bytes_with_container_id(&input, [0xa3; 16], options(128, 128))
            .expect("sparse container should pack");
    let dense_container = pack_bytes_with_dense_line_index(&input, [0xa4; 16], options(128, 128))
        .expect("dense container should pack");
