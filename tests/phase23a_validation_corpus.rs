use qzt::{
    Checksum, DocumentIndex, QztError, QztFileReader, QztReader, VerifyLevel,
...
fn c1_document_index(corpus: &[u8]) -> DocumentIndex {
    DocumentIndex {
        ...
        documents: vec![document(&DocumentFixture {
            ...
            input: corpus,
            logical_offset: 0,
            byte_length: u64::try_from(corpus.len()).expect("fixture fits in u64"),
            first_line: 0,
            line_count: line_count_for(corpus),
            // Writer options size the single-chunk pack so chunk indices stay
            // deterministic (0..1) for this smoke fixture.
            chunk_start: 0,
            chunk_end: 1,
        })],
    }
...
fn c1_conversation_pack_export_document_index_and_utf8_boundary_smoke() {
    ...
    let container = pack_bytes_with_document_index(
        ...
        writer_options(chunk_size, chunk_size),
        &document_index,
    )
