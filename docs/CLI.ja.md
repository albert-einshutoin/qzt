...
- `attest`は検証成功まで何も書かず、失敗時stdoutは空です。
...
file入力も64 KiBずつ読み、`<output>.tmp`からrenameします。それ以外は入力全体を
memoryへ読みます。`qzt pack --profile memory`は必要なDocument Indexを作れないため
...
全原文byteをstdout、または新規作成/切詰めする出力fileへstreamします。chunk復号時に
readerのnormal integrity checkを行います。
...
`incomplete_reason`は現在`query_shorter_than_ngram_n`または
`query_has_no_indexable_tokens`です。null以外なら、空/部分結果を完全な否定結果として
解釈してはいけません。
...
正準JSON 1行だけを書きます。[アテステーション正準形](#アテステーション正準形)と
...
| `core` | 既定。Dense offの単一入力packはbounded streaming writer。 |
