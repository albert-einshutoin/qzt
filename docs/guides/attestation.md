...
The checks below target the [pinned development CLI](../../README.md#development-cli)
at `ad709214f1e8ae18eff6e9f0b633345e40d1617b`. The published
[`v0.1.0-pre.2` binary](https://github.com/albert-einshutoin/qzt/releases/tag/v0.1.0-pre.2)
emits a versionless attestation with only `verify.level`, `checked_chunks`, and
`decoded_bytes`; it cannot satisfy the v1 policy below. Use the
[release tour](../../README.md#60-second-tour) for its actual output.

The development CLI's default `attestation_schema` is `qzt-attestation-v1`. The `format` value
