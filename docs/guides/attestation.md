...
The new default `attestation_schema` is `qzt-attestation-v1`. The `format` value
...
To compare a saved legacy v0 attestation byte for byte, build the CLI from the
commit that generated it, or from the last pre-#292 main commit
`aaaff662f4b84c1389a4aaffb1087e6c6ef88fe1`, then run its `qzt attest`
with the original level and QZT file and compare that output with the saved
