...
以下の例では `qzt` を `PATH` にある前提で示します。ローカルビルドから実行する場合は
`./target/release/qzt` に置き換えてください。
...
JSON 出力（`--format json`）: `qzt info`、`qzt verify`、`qzt docs`、
`qzt search` は stdout に単一の JSON object を出力します。`qzt verify --format json`
は成功・失敗どちらでも stderr を出しません（失敗時は JSON の `"ok":false` に
エラーを含めます）。`qzt search --format json` は stdout を parse 可能に保ち、
incomplete-result の警告だけ stderr に出します。`qzt doc` は document bytes を
出力するコマンドで JSON ではありません。`doc_id` の一覧は `qzt docs --format json`
を使ってください。未知の `--format` 値は exit **2** です。
...
decode 前の作業量を抑えるには `--max-candidate-granules <N>`（候補 granule 数）と
`--max-decoded-bytes <N>`（decode バイト数。`KiB` / `MiB` / `GiB` 接尾辞可）も指定できます。
上限に達した場合も結果上限と同様に `capped=true` で **exit 0** です。
