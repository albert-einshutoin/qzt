...
現在のbinary配布は`v0.1.0-pre.1` technical previewです。macOS / Linuxでは、
生成済みinstallerが実行環境に合うarchiveを選択します。
...
Windowsでは同じReleaseの`.zip`と`.zip.sha256`、または
`qzt-installer.ps1`を利用できます。プリビルドbinaryを使わず、review済みtagから
buildして導入する場合:
